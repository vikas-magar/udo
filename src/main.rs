...
async fn main() -> Result<()> {
    ...
    if let Some(Commands::Server) = cli.command {
        start_server("udo_metrics.duckdb").await.context("Failed to start server")?;
        return Ok(());
    }
    ...
    let (source, processors, sink_factory, dlq, batch_size, input_for_schema) = if let Some(config_path) = args.config {
        ...
        let config: udo::core::config::PipelineConfig = serde_yaml::from_str(&config_str).context("Failed to parse YAML config")?;
        ...
        let source: Box<dyn InputSource> = match config.source {
            ...
            #[cfg(feature = "kafka")]
            udo::core::config::SourceConfig::Kafka { brokers, group_id, topic } => {
                Box::new(udo::io::source::KafkaSource::new(&brokers, &group_id, &topic)?)
            }
        };
        ...
        let sink_factory: Box<dyn Fn(Arc<Schema>) -> Result<Box<dyn udo::core::pipeline::OutputSink>> + Send + Sync> = match config.sink {
            udo::core::config::SinkConfig::File { path } => {
                Box::new(move |s| Ok(Box::new(udo::io::sink::ParquetSink::new(path.clone(), s).map_err(|e| udo::UdoError::Pipeline(e.to_string()))?)))
            }
            #[cfg(feature = "cloud")]
        ...
        let dlq: Option<Box<dyn udo::core::pipeline::DlqSink>> = if let Some(dlq_cfg) = config.dlq {
            match dlq_cfg {
                udo::core::config::SinkConfig::File { path } => {
        ...
        // Legacy CLI behavior
        let input_path_str = args.input.context("Input is required if no config file provided")?;
        let output_path = args.output.context("Output is required if no config file provided")?;
        
        let source: Box<dyn InputSource> = if input_path_str.starts_with("kafka://") {
            #[cfg(feature = "kafka")]
            {
                let parts: Vec<&str> = input_path_str.trim_start_matches("kafka://").split('/').collect();
                if parts.len() < 3 { bail!("Invalid Kafka URL"); }
                Box::new(udo::io::source::KafkaSource::new(parts[0], parts[1], parts[2])?)
            }
        ...
        let sink_factory: Box<dyn Fn(Arc<Schema>) -> Result<Box<dyn udo::core::pipeline::OutputSink>> + Send + Sync> = Box::new(move |s| {
            #[cfg(feature = "cloud")]
            {
        ...
        let schema_input = if !input_path_str.starts_with("kafka://") { Some(input_path_str) } else { None };
        (source, procs, sink_factory, None, args.batch_size, schema_input)
    };
    ...
    if let Some(inp) = input_for_schema {
        if args.scan_rows > 0 {
            ...
            if !schema_rows.is_empty() {
                ...
                schema = Some(Arc::new(udo::core::schema::infer_schema(&schema_array, None).map_err(|e| anyhow::anyhow!(e))?));
            }
        }
    }
